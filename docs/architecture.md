...
## Gate 385 — One-form support and edge-measure selection

Gate 385 upgrades the Gate-384 conditional edge-measure lane into a finite support theorem.

The scalar Higgs channel in the almost-commutative spectral action is a finite inner fluctuation. In ASHA notation:

```text
A_F = Σ a[D_F,b]
```

This places the Higgs in the represented one-form module `Ω¹_D(A_F)`. Since one-forms are generated by commutators with the finite Dirac operator, their support is the finite interaction graph. The relevant support is therefore the ten `J`-doubled finite Dirac edge slots rather than the seven contact nodes.

The canonical one-form inner product restricts the Hilbert-space trace to that support:

```text
A = P_E A P_E
⟨A,B⟩ = Tr_HF(A†B) = Tr_E(A†B)
```

Combined with Gate 384, this selects the raw edge-measure trace ratio

```text
(e/a²)_edge = (7/10)(e/a²)_node
```

and yields the clean Higgs coefficient identity
...
Gate 400 tests the non-quaternionic route requested by Gate 399: whether a mixed one-form edge/contact invariant supplies the missing basis-free scalar identity selector. It formalizes `Delta_E = D_F^2` on the J-doubled one-form edge support, then audits contact/scalar compressions on the four-real `H_phi` carrier. The result is conservative: the uniform edge Laplacian is central on `H_phi`, the raw `P_C Delta_E P_K` compression is not an `H_phi` endomorphism, and the squared compression recovers the known pair-degenerate scalar response rather than the irreducible contact quartic `q4`. A sealed `q4` companion matrix can be placed on `H_phi`, but only by arbitrary basis insertion, so it is quarantined. The next required object is a derived nonuniform edge-weight operator, not another dimension or polynomial coincidence.

## Gate 401 — Derived Edge-Weight Operator / Hypercharge Laplacian Sieve

Gate 401 tests whether the missing scalar identity selector can be produced by differentiating the one-form edge Laplacian with native charge weights. The audited edge graph has five structural finite-Dirac edge classes and ten J-doubled one-form slots. Hypercharge, B-L, and T3-like scalar branch signs are native and require no observed fermion masses or fitted Yukawa amplitudes. However, the canonical scalar quotient compresses these charge weights by Higgs branch and remains either central or pair-degenerate. Edge-resolved charge weights have degree-four capacity only after choosing a noncanonical edge-to-real-component basis for `H_phi`, and their characteristic polynomials do not match the irreducible contact quartic `q4`. The gate therefore preserves the Gate-400 obstruction: `q4` remains an exact contact spectral datum, while `H_phi` remains the mature scalar/contact one-form carrier. The next route is a spectral graph edge-adjacency/incidence Laplacian that is native to the one-form edge graph itself.

### Gate 402 — Spectral graph edge-adjacency operator search

Gate 402 tests the topology of the one-form edge graph itself after Gate 401 rejected gauge-charge-weighted Laplacians as `q4` selectors. The canonical four Yukawa edges form `K2 ⊔ K2`, giving pair-degenerate adjacency/Laplacian spectra and therefore no irreducible quartic on `H_phi`. The full structural graph with the sterile/Majorana edge forms `P3 ⊔ K2`, which has quartic minimal-polynomial capacity, but it is a five-edge graph operator rather than a canonical four-real scalar endomorphism; its quartic `x(x-1)(x-2)(x-3)` is polynomially disjoint from the contact primary `q4`. J-doubling only duplicates this structural spectrum. Thus the edge-graph lane is real but unsealed: the scalar/contact identity still requires either an oriented incidence/boundary theorem or a canonical edge-to-`H_phi` quotient, and the 13 charged flavor moduli remain untouched.
...
### Gate 406 — Contact-eigenoperator internal reconstruction / q4 contact-only classification

Gate 406 follows the Gate-405 pullback obstruction and closes the current `q4 -> H_phi` route. Rather than adding another scalar or edge projection, it reconstructs `q4` internally as the companion/eigenoperator of the contact spectral primary module `Q[x]/(q4)`. The characteristic and minimal polynomial match the irreducible contact quartic exactly, and the construction uses no scalar basis, edge basis, Yukawa amplitude, CKM/PMNS parameter, observed mass, or empirical fitting input.

The contact algebra audit then explains why this does not become a scalar theorem. Since `q4` is irreducible over `Q`, the rational centralizer is the field `Q[C_q4]` and contains no nontrivial idempotent projector. A 2+2 split, root-pairing, or root-sector semantics would require a resolvent-field adjunction and a selected branch, which remains sealed. Gates 398-405 already rejected direct scalar, quaternionic, mixed-Laplacian, weighted-edge, graph, oriented-incidence, quotient, and pullback identifications. Gate 406 therefore classifies `q4` as a contact-only spectral invariant under the current ASHA functor inventory. The scalar/contact `H_phi` lane remains real but separate, no Yukawa couplings are reduced, and the thirteen charged flavor moduli remain firewalled.

### Gate 407 — Hphi-native scalar selector algebra / pair-degeneracy closure sieve

Gate 407 follows the Gate-406 contact-only classification of `q4` and restarts the scalar problem inside the correct jurisdiction: the native endomorphism algebra of `H_phi`. The audited generators are the weak quaternionic left/right actions, the pair-degenerate scalar response, and the canonical one-form/Yukawa edge quotient. No contact companion operator, root ordering, observed fermion mass, CKM/PMNS datum, or Yukawa amplitude is imported.
